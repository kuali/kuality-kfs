class ObjectCodePage < BasePage
  ...
  #field missing but listed
  # element(:reports_to_chart_code) { |b| b.frm.text_field(name: '') }
  ...
  element(:active_indicator) { |b| b.frm.check_box(name: 'document.newMaintainableObject.active') }
  element(:budget_aggregation_code) { |b| b.frm.text_field(name: 'document.newMaintainableObject.financialBudgetAggregationCd') }

  element(:mandatory_transfer) { |b| b.frm.select(title: 'Search Budget Aggregation Code') }
